...
if (!authStore.checkAdmin) throw new Error('You must be an admin to remove a category.')

const handleRemove = async () => {
  const { error } = await supabase.from('categories').delete().eq('id', props.categoryId)
  if (error) {
    ...
    <AlertDialogPortal>
      ...
      >
        ...
        <AlertDialogDescription class="text-mauve11 mt-4 mb-5 text-[15px] leading-normal">
          This action cannot be undone. This will permanently delete the category and remove it from
          our servers.
        </AlertDialogDescription>
